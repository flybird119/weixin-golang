...
func WithdrawList(model *pb.StoreWithdrawalsModel) (models []*pb.StoreWithdrawalsModel, err error, totalCount int64) {
	query := "select w.id,w.card_type,w.card_no,w.card_name,w.username,w.withdraw_fee,w.status,w.apply_phone,extract(epoch from w.apply_at)::integer,extract(epoch from w.complete_at)::integer ,extract(epoch from w.accept_at)::integer from withdrawals w where 1=1"
	queryCount := "select count(*) from withdrawals where 1=1"
	...
	for rows.Next() {
		...
		err = rows.Scan(&withdraw.Id, &withdraw.CardType, &withdraw.CardNo, &withdraw.CardName, &withdraw.Username, &withdraw.WithdrawFee, &withdraw.Status, &withdraw.ApplyPhone, &withdraw.ApplyAt, &complete_at, &accept_at)
		if err != nil {
			log.Error(err)
			return
		}
		...
		query = fmt.Sprintf("select s.name,a.balance from store s join account where s.id=a.store_id where s.id='%s'", withdraw.StoreId)
		log.Debug(query)
